...
const Home = () => {
  ...
  const [bookings, setBookings] = useState<
    { detail: string; name: string; time: string[]; ymd: string }[]
  >([]);
  ...
  const toggleALL = (value: string) => {
    switch (value) {
      case "午前":
        ...
        {
          timeAM.map((time) => toggleBgColor(selectedDay, time));
        }
      ...
      case "午後":
        ...
        {
          timePM.map((time) => toggleBgColor(selectedDay, time));
        }
      ...
      default:
        ...
        {
          timeALL.map((time) => toggleBgColor(selectedDay, time));
        }
    }
  ...
  const bookingreload = () => {
    // matchingUsersを更新
    const updatedMatchingUsers = bookings.filter(
      (booking: BookingUser) => booking.ymd === selectedDay
    );
          ...
          <div className=" w-3/12 h-full border-l-2 border-gray-200 fixed right-0 bg-gray-200">
            ...
            <div className="h-auto w-full mt-3">
              <ul className="flex flex-wrap">
                <li
                  onClick={() => handleTabClick("ng")}
                  className={`w-1/2  p-1 text-center cursor-pointer rounded-l-lg shadow-lg border ${
                    selectedTab === "ng"
                ...
                <li
                  onClick={() => (handleTabClick("book"), bookingreload())}
                  className={`w-1/2  p-1 text-center cursor-pointer rounded-r-lg shadow-lg border${
                    selectedTab === "book"
                      ? "border-sky-700 bg-sky-700 text-white"
                      : "border-gray-200 bg-gray-50"
                  }`}
                ...
                {selectedTab === "ng" && (
                  <div className="fixed p-1 w-auto h-auto">
                      ...
                      {timeAll.map((time) => (
                        <div
                          ...
                          className={`border border-black hover:border-gray-400 rounded-lg flex items-center justify-center w-16 h-10 m-2 font-bold ${
                            isTimeIncluded(selectedDay, time)
                              ? "bg-gray-400"
                              : "bg-white"
                          }`}
                    ...
                    <div className="text-center">
                      <button
                        onClick={handleClick}
                        className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 mt-4 mb-2 border border-blue-700 rounded"
                      >
                        保存
                      </button>
                    </div>
